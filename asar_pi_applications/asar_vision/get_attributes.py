...
def get_attributes(image):
    ...
    thresh_points = [60, 70, 80, 90, 100, 110, 120, 130, 140, 150]
    ...
    blurred = cv2.blur(gray, (4, 4), 0)
    for k in range(0, len(canny_points)):
        thresh = cv2.threshold(blurred, thresh_points[k], 255, cv2.THRESH_BINARY)[1]
        canny_edge = cv2.Canny(blurred, canny_points[k], 40)
        cv2.imshow('thresh', canny_edge)
        cnts = cv2.findContours(canny_edge, cv2.RETR_EXTERNAL,
    ...
    for i in range(0, len(hexagon_attributes['coordinate'])):
        if hexagon_attributes['coordinate'][i] not in final_list_coordinate:
            final_list_coordinate.append(hexagon_attributes['coordinate'][i])
            final_list_color.append(hexagon_attributes['color'][i])
