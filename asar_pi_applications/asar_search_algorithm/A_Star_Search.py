...
import asar_search_algorithm.search_constants as SEARCH_CONSTANTS
import base64
import logging
...
TERRAIN_HEIGHT = SEARCH_CONSTANTS.TERRAIN_HEIGHT  # number of tiles down the side
TERRAIN_WIDTH = SEARCH_CONSTANTS.TERRAIN_WIDTH  # number of tiles across the top
INFINITY = SEARCH_CONSTANTS.INFINITY  # variable representing impassable terrain
...
def instantiate_graph():
    ...
    for x in range(1, TERRAIN_HEIGHT + 1):
        for y in range(1, TERRAIN_WIDTH + 1):
            ...
            children = filter(lambda i: 1 <= i[0] <= SEARCH_CONSTANTS.TERRAIN_HEIGHT and
                                        1 <= i[1] <= SEARCH_CONSTANTS.TERRAIN_WIDTH,
                              temp_child)
...
def a_star_search(tile, start, goal, mode):
    ...
    while not frontier.empty():
        ...
        for i in range(0, len(tile[current]['children'])):
            next_node = tile[current]['children'][i]  # iterates through children of current node
            if tile[next_node] == goal:
                priority = cost_so_far[current] + 1 + SEARCH_CONSTANTS.heuristic(goal, next_node)
                frontier.put(next_node, priority)
                came_from[next_node] = current
                break
            else:
                # heuristic part 1 (uses danger & Speed)
                new_cost = cost_so_far[current] + SEARCH_CONSTANTS.travel(next_node, tile, mode)
                if next_node not in cost_so_far or new_cost < cost_so_far[next_node]:  # if next has not been visited
                    if tile[next_node]['speed'] < INFINITY:  # do not evaluate obstacles (Rock & water)
                        if mode == 0 and tile[next_node][
                                'immediate_danger'] >= SEARCH_CONSTANTS.SAFE_LIMIT:
                            # do not drive through fire in safe mode
                            continue
                        else:
                            # heuristic part 2 (Euclidean distance)
                            cost_so_far[next_node] = new_cost
                            priority = new_cost + SEARCH_CONSTANTS.heuristic(goal, next_node)
                            log.info('{0} --> {1} = {2}'.format(current, next_node, priority))
                            frontier.put(next_node, priority)  # put evaluated node onto queue
                            came_from[next_node] = current
...
def path_to_move(path, tile):
    ...
    for i in range(1, len(path)):
        ...
        if tile[path[i]]['speed'] == 0:  # fast
            speed = SEARCH_CONSTANTS.FAST
        elif tile[path[i]]['speed'] == 1:  # medium
            speed = SEARCH_CONSTANTS.MEDIUM
        else:  # default speed for unknown objects == slow
            speed = SEARCH_CONSTANTS.SLOW
...
def run_search(graph_filepath, speed_mode):
    """
    Main function for calling by external code that takes a filepath of nodes
    and returns a filepath to encoded robot directions.
    :return:
    """
    start_node, goal_node, tile = SEARCH_CONSTANTS.give_danger(instantiate_graph(), graph_filepath)
    node_came_from, start_node, goal_node = a_star_search(tile, start_node, goal_node, speed_mode)
    output_path = reconstruct_path(node_came_from, start_node, goal_node)
    return path_to_move(output_path, tile)


def main():
    start, goal, tile = SEARCH_CONSTANTS.give_danger(instantiate_graph())
    came_from, start, goal = a_star_search(tile, start, goal, SEARCH_CONSTANTS.MODE)
